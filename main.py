...
# Increase timeout — PostHog HogQL queries can take a while on larger datasets
POSTHOG_TIMEOUT      = int(os.environ.get("POSTHOG_TIMEOUT", "90"))
...
WHERE event = 'User Signup'
  ...
  AND person.id NOT IN (
        ...
        WHERE event = 'StripeV2 Trial Started'
  )
  AND person.id NOT IN (
        SELECT DISTINCT person_id FROM events
        WHERE event = 'StripeV2 Credit Card Added'
  )
GROUP BY email, name
...
def push_to_instantly(campaign_id: str, leads: list) -> dict:
    ...
    uploaded, skipped = _push_leads_individually(campaign_id, leads)
    return {"uploaded": uploaded, "skipped": skipped}

def _push_leads_individually(campaign_id: str, leads: list) -> tuple:
    uploaded = skipped = 0
    for lead in leads:
        ...
        try:
            ...
            if resp.ok:
                ...
                # Already in workspace — not an error, just a duplicate
                skipped += 1
            else:
                log.warning(
...
def main():
    ...
    # 1. Fetch
    # IMPORTANT: transient PostHog errors (timeouts, 5xx) exit with code 0.
    # Exit code 0 = clean exit; Railway will NOT send a crash alert.
    # Exit code 1 = crash; Railway WILL send a crash alert.
    # We only want alerts for genuine, unexpected failures — not API blips.
    try:
        rows = fetch_notrial_signups(LOOKBACK_HOURS)
    except Exception as exc:
        log.error("PostHog fetch failed (transient — will retry next run): %s", exc)
        logging.shutdown()
        sys.exit(0)
